...
echo "$($PSQL "SELECT name FROM games as g FULL JOIN teams as t ON g.winner_id = t.team_id OR g.opponent_id = t.team_id WHERE year = 2014 AND round = 'Eighth-Final' order by name")"
